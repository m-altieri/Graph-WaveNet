...
argparser.add_argument(
    "-o", "--output", action="store", help="set the name of the output folder"
)
argparser.add_argument(
    "-m",
    "--mode",
...
log_file_name = "{}-{}-{}.log".format(
    args.model, args.dataset, datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
)
...
def create_timeseries(
    ...
    logger.warning(
        "Calculating training, validation and test splits ("
        + str(train_ratio)
        + ", "
        + str(val_ratio)
        + ", "
    ...
    if test_source == "random":
        ...
        test_indexes = (
            np.load(test_file_path) - h // stride
    ...
    if not allow_partial_batches:
        logger.warning(
            "Making number of samples a multiple of batch size ("
            + str(batch_size)
            + ")..."
        )
...
def build_model(
    ...
    **kwargs,
    ...
    logger.warning(
        "Starting model construction for model type {}....".format(model_name)
    )
    ...
    if model_name == "GraphWaveNet":
        ...
        model = ESG(
            ...
            gcn_depth=2,
            device="cuda:0",
            fc_dim=16 * history_steps,  # lightsource funziona con 16,  # era 95744,
...
def get_optimizer(model_name, model, lr):
    ...
    if model_name == "GraphWaveNet":
        ...
        optimizer = torch.optim.Adam(
            params=model.parameters(), lr=lr, weight_decay=0, eps=1.0e-8, amsgrad=False
        )
...
def train_and_predict(
    ...
    **kwargs,
    ...
    logger.warning(str(torchinfo.summary(model, input_size=trainX[:1].shape)))
    ...
    for i, index in enumerate(test_indexes):
        ...
        start_index = min(
            index - math.ceil((index - last_index) / batch_size) * batch_size,
            index - math.ceil(replay / batch_size) * batch_size,
        )
        if start_index < 0:
            start_index += batch_size  # Fix per un problema per cui può andare in negativo per la prima data

        logger.warning("Training on [%d, %d]" % (start_index, index))
        if dynamic_adj:
            model.set_adj(create_adj(sequence=trainX[start_index:index]))

        # x = trainX[start_index: index]
        # y = trainY[start_index: index]

        dataset = torch.utils.data.TensorDataset(
            torch.Tensor(trainX[start_index:index]),
            torch.Tensor(trainY[start_index:index]),
        )
        ...
        for epoch in range(args.epochs):
            ...
            for data in training_loader:
                ...
                if model_name == "GraphWaveNet" or model_name == "MTGNN":
                    ...
                    pred = pred.mean(
                        dim=-1
                    )  # comprimo l'ultima dimensione da 12: [B,T,N]
                    mae, mape, rmse = gwnet_util.calc_metrics(
                        pred.squeeze(1), y, null_val=0.0
                    )
                    mae.backward()
                    ...
                    pred = model(x, x, dec_inp, dec_inp)
                    # pred = model(x, None, None, None)
...
class ExperimentRunner:
    ...
    def is_finished(folder, run_name):
        ...
        return os.path.exists(
            os.path.join(PATH, "experiments", folder, "recap-{}.csv".format(run_name))
        )
    ...
    def collect_preds(self):
        ...
        for dataset in self.datasets:
            df_path = os.path.join(
                PATH, "experiments", self.name, "{}.csv".format(dataset)
            )
        ...
        for dataset in self.datasets:
            df_path = os.path.join(
                PATH, "experiments", self.name, "recap-{}.csv".format(dataset)
            )
    ...
    def _run(self, model_name, dataset_name, run_name):
        ...
        if not os.path.exists(os.path.join(self.results_path, self.name)):
            os.mkdir(os.path.join(self.results_path, self.name))
        ...
        preds_path = os.path.join(
            self.results_path, self.name, "preds-" + run_name + ".csv"
        )
        attw_path = os.path.join(
            self.results_path, self.name, "attw-" + run_name + ".npy"
        )
        int_path = os.path.join(
            self.results_path, self.name, "int-" + run_name + ".npz"
        )

        logger.warning(
            "Training starting for model {} and dataset {}....".format(
        ...
        dataset_p = self.dataset_params[
            "default"
        ].copy()  # Initialize with default params
        ...
        timeseries_file_path = os.path.join(
            PATH, "data", dataset_name, dataset_name + ".npz"
        )
        ...
        if not pred_seqs_file_suffix:
            raise AttributeError(
                "Can't find a value for prediction_seqs_file. If the program was run from command line, probably no value was passed for flag -d."
            )
        ...
        test_file_path = os.path.join(
            ...
            "data",
            dataset_name,
            dataset_name + "_" + experiment_params["pred_seqs_file_suffix"] + ".npy",
        )
        ...
        checkpoint_path = None
        if checkpoint:
            checkpoint_path = os.path.join(PATH, "checkpoints", model_name)
        ...
        adj_file_path = os.path.join(
            PATH, "data", dataset_name, adj_type + "-" + dataset_name + ".npy"
        )
        ...
        if single_node_model:
            preds_merged = np.empty(
                (nodes * multipreds[0].size,), dtype=multipreds[0].dtype
            )
...
if args.conf and not args.all:
    ...
    for model in model_params["models"]:
        if model["alias"] == args.model:
            finished = ExperimentRunner.is_finished(
                args.output, "{}-{}".format(model["name"], args.dataset)
            )
            if args.skip_finished and finished:
                logger.critical(
                    "Run {}-{} will be skipped, as it is already finished and flag --skip-finished is set.".format(
                        model["name"], args.dataset
                    )
                )
                continue
