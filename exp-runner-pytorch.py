...
def train_and_predict(model, trainX, trainY, valX, valY, testX, testY, test_indexes, mode, optimizer, nodes, features, history_steps, prediction_steps, adj,
    ...
    for i, index in enumerate(test_indexes):
        ...
        training_loader = torch.utils.data.DataLoader(list(zip(trainX[start_index: index], trainY[start_index: index])), batch_size=4, shuffle=False, num_workers=1)
        ...
        logger.info(f'Predicting on {np.expand_dims(np.transpose(testX[i], (0,2,1)), 0).shape}')
        pred = model(np.expand_dims(np.transpose(testX[i], (0,2,1)), 0))
