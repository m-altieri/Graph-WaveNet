...
class TConv(nn.Module):
    def __init__(
        ...
        self.filter_conv = Dilated_Inception(
            residual_channels, conv_channels, kernel_set, dilation_factor
        )
...
class Extractor(nn.Module):
    def __init__(
        ...
        self.t_conv = TConv(
            residual_channels, conv_channels, kernel_set, dilation_factor, dropout
        )
        self.skip_conv = nn.Conv2d(conv_channels, skip_channels, kernel_size=(1, t_len))

        self.s_conv = Evolving_GConv(
            conv_channels,
            residual_channels,
            gcn_depth,
            st_embedding_dim,
        ...
        self.residual_conv = nn.Conv2d(
            conv_channels, residual_channels, kernel_size=(1, 1)
        )
    ...
    def __init__(
        ...
        for i in range(1, n_layers + 1):
            if dilation_exp > 1:
                rf_size_i = int(
                    rf_block
                    + (kernel_size - 1) * (dilation_exp**i - 1) / (dilation_exp - 1)
                )
            else:
...
class ESG(nn.Module):
    def __init__(
        ...
        self.skip0 = nn.Conv2d(
            in_dim, skip_channels, kernel_size=(1, self.total_t_len), bias=True
        )
        self.skipE = nn.Conv2d(
            residual_channels,
            skip_channels,
            kernel_size=(1, self.total_t_len - self.receptive_field + 1),
            bias=True,
        )
    ...
    def forward(self, input):
        ...
        x = x.reshape(b, self.pred_len, -1, n).transpose(
            -1, -2
